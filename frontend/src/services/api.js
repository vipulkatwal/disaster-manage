...
const officialUpdatesApi = {
	getForDisaster: async (disasterId, params = {}) => {
		try {
			...
			const response = await api.get(
				`/disasters/${disasterId}/official-updates`,
				{ params }
			);
	...
	getByCategory: async (category, params = {}) => {
		try {
			...
			const response = await api.get(`/official-updates/category/${category}`, {
				params,
			});
				...
				results: [
					{
						...
						content:
							"New emergency shelters have been opened in Manhattan and Brooklyn. Capacity for 500+ people available. All shelters are equipped with basic necessities and medical support.",
						url: "https://fema.gov/disaster-updates",
						published_at: new Date(
					...
					},
					{
						...
						content:
							"Water distribution is now active at Central Park and Prospect Park locations from 8 AM to 6 PM. Free bottled water and emergency supplies available.",
						url: "https://nyc.gov/emergency",
						published_at: new Date(
							Date.now() - 2 * 60 * 60 * 1000
						).toISOString(),
					...
					},
					{
						...
						content:
							"Red Cross is accepting volunteer registrations for disaster relief efforts. Training provided for all volunteers. Multiple shifts available.",
						url: "https://redcross.org/volunteer",
						published_at: new Date(
							Date.now() - 1 * 60 * 60 * 1000
						).toISOString(),
					...
					},
					{
						...
						content:
							"Severe weather conditions expected to continue through tomorrow evening. Stay indoors and avoid unnecessary travel.",
					...
					},
					{
						...
						content:
							"Mobile food units are serving hot meals in affected areas. Check locations on our website. Meals available 24/7.",
...
export const useApi = () => ({
	...
	resources: {
		getNearby: async (disasterId, params = {}) => {
			try {
				const response = await api.get(`/disasters/${disasterId}/resources`, {
					params,
				});
				return { success: true, data: response.data };
			} catch (error) {
		...
		create: async (disasterId, data) => {
			try {
				const response = await api.post(
					`/disasters/${disasterId}/resources`,
					data
				);
				return { success: true, data: response.data };
			} catch (error) {
	...
	verification: {
		verifyImage: async (disasterId, data) => {
			try {
				const response = await api.post(
					`/disasters/${disasterId}/verify-image`,
					data
				);
				return { success: true, data: response.data };
			} catch (error) {
