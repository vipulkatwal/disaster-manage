...
export const useApi = () => {
	...
	const disasters = {
		...
		getReports: useCallback(
			(id, params = {}) => {
				...
				const url = queryString
					? `/disasters/${id}/reports?${queryString}`
		...
		getSocialMedia: useCallback(
			(id, params = {}) => {
				const queryString = new URLSearchParams(params).toString();
				const url = queryString
					? `/disasters/${id}/social-media?${queryString}`
		...
		getOfficialUpdates: useCallback(
			(id) => {
	...
	const socialMedia = {
		getReports: (disasterId, params = {}) => {
			...
			const url = qs
				? `/disasters/${disasterId}/social-media?${qs}`
				: `/disasters/${disasterId}/social-media`;
			return get(url);
		},
		getMockData: (params = {}) => {
			const qs = new URLSearchParams(params).toString();
	...
	const officialUpdates = {
		getForDisaster: (disasterId, params = {}) => {
			const qs = new URLSearchParams(params).toString();
			const url = qs
				? `/disasters/${disasterId}/official-updates?${qs}`
		...
		getByCategory: (category, params = {}) => {
			const qs = new URLSearchParams(params).toString();
			const url = qs
				? `/official-updates/category/${category}?${qs}`
				: `/official-updates/category/${category}`;
			return get(url);
		},
		search: (params = {}) => {
			const qs = new URLSearchParams(params).toString();
			const url = qs
				? `/official-updates/search?${qs}`
				: "/official-updates/search";
			return get(url);
		},
	};
	...
	const resources = {
		getNearby: useCallback(
			(disasterId, params = {}) => {
				const queryString = new URLSearchParams(params).toString();
				const url = queryString
					? `/disasters/${disasterId}/resources?${queryString}`
		...
		async (requests) => {
			...
			try {
				...
				if (failureCount > 0) {
					...
					toast.success(
						`All ${responses.length} requests completed successfully`
					);
				}
