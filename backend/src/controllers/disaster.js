	...
	try {
		...
		if (!location_name && description) {
			try {
				...
				logger.warn(
					"Failed to extract location from description:",
					error.message
				);
			}
		}
...
const updateDisaster = async (req, res) => {
	try {
		const { id } = req.params;
		const { title, location_name, description, tags, priority, status } =
			req.body;
...
const createReport = async (req, res) => {
	try {
		...
		const { content, image_url, priority = "low", tags = [] } = req.body;
		const user_id = req.user.id;
		...
		req.io.emit("report_created", { disaster_id, report: data });
...
const getReports = async (req, res) => {
	try {
		const { id: disaster_id } = req.params;
		const { verification_status, priority, limit = 50, offset = 0 } = req.query;
