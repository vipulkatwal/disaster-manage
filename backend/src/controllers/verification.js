	...
	try {
		...
		if (report_id) {
			const { error: updateError } = await supabase
				.from("reports")
				.update({
					verification_status: verificationResult.is_authentic
						? "verified"
						: "flagged",
		...
		logger.info(
			`Image verification completed for disaster ${disaster_id}: ${
				verificationResult.is_authentic ? "VERIFIED" : "FLAGGED"
			}`
		);
...
const getVerificationHistory = async (req, res) => {
	try {
		...
		const { data, error } = await supabase
			.from("reports")
			.select(
				"id, image_url, verification_status, verification_result, created_at"
			)
			.eq("disaster_id", disaster_id)
